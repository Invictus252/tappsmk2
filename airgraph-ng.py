...
class dotCreate:
    ...
    def CAPR(self,storedScan_):
        ...
        for bssid in list(self.capr.keys()):
            ...
            for client in self.capr[bssid]:
                ...
                self.dotFile.append(self.clientColor(storedScan_,client,'black',[self.client[client]['lts'],self.client[client]['fts']],self.client[client]['power'],None,True))
            self.dotFile.append(
                self.apLabel(bssid,Color,len(self.capr[bssid]),time))
        footer ='label="Generated by Tapps\\n%s Access Points and\\n%s Clients shown";\n}' %(tap,tclient)
        ...
        for key in list(self.client.keys()):
            ...
            if sdata["probe"] != ['']:
                ...
                for probe in sdata['probe']:
                    ...
                    self.dotFile.append(self.clientColor(storedScan_,key,'black',[lts,fts],None,True))
    ...
    def clientColor(self,storedScan_,mac,color,time=None,power=None,label=None,DouiCheck=False):
        """
        ...
        if DouiCheck == True:
            ...
            MHz = 2417
            FSPL = 27.55
            dist = 10 ** ((FSPL - (20 * log10(MHz)) + abs(int(power))) / 20)
            dist = round((dist * 3.281), 2)
            scan = storedScan_

            mycursor = mydb.cursor()
            mycursor.execute('INSERT IGNORE INTO ScanResults (DeviceName, Mac, OUI, Power, Distance, FirstTimeSeen, LastTimeSeen, ScanGroup) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)', ('BRAVO',mac,self.APouiLookup(mac), int(pwr), float(dist), fts, lts,scan))

            mydb.commit()
            rtn = '\t"%s" [label="%s\\nOUI: %s\\nDevice Type: %s\\nPower: %s\\nFirst Time Seen: %s\\nLast Time Seen: %s" color=%s fontsize=9];\n' %(mac,mac,self.APouiLookup(mac),self.clientOuiLookup(mac),pwr,fts,lts,color)
        else:
    ...
    def apLabel(self,bssid,color,cnum,time):
        ...
        fts = time[1]
        AP = self.Ap[bssid]
        return'\t"%s" [label="%s\\nEssid: %s\\nChannel: %s\\nEncryption: %s\\nOUI: %s\\nPower: %s\\nFirst Time Seen: %s\\n Last Time Seen: %s\\nNumber of Clients: %s" style=filled fillcolor="%s" fontcolor="%s" fontsize=9];\n' %(bssid,bssid,AP['essid'].rstrip('\x00').replace('"', '\\"'),AP['channel'],AP['privacy'],self.APouiLookup(bssid),AP['power'],fts,lts,cnum,color[0],color[1])
    ...
    def graphCreate(self,keepdot_):
        ...
        try:
            subprocess.Popen(["fdp","-Tpng",self.dot,"-o","./public/img/" + self.png,"-Gcharset=latin1"]).wait()
        except Exception:
            os.remove(self.dot)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s",
        "--scan",
        dest="scan",
        help="Scan Counter from web app")
